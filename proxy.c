 ...
 * Assignment 2
 * Student Name: David Lambropoulos
 * 
...
int main(int argc, char **argv)
{
	...
	int connfd; //File descriptor for connections
	int serverfd; //File descriptor for the server
	int len;	//arbitrary variable
	struct sockaddr_in cliaddr; //structure for client address
	struct sockaddr_in servaddr; //structure for server address
	struct hostent *hp;	//Holds a pointer to the host
	char *haddrp;
	rio_t rio;
	rio_t rio2;
	char dummyBuffer[MAXBUF];   //Getting GET information back from browser
	char dummyBuffer2[MAXBUF];  //Getting Host: information back from browser
	char dummyBuffer3[MAXBUF];  //Getting User-Agent: information back from browser
	char dummyBuffer4[MAXBUF];  //Getting Accept: information back from browser
	char dummyBuffer5[MAXBUF];  //Getting Accept-Language: information back from browser
	char dummyBuffer6[MAXBUF];  //Getting Accept-Encoding: information back from browser
	char dummyBuffer7[MAXBUF];  //Getting Cookie: information back from browser
	char dummyBuffer8[MAXBUF];  //Getting Connection: information back from browser
	char fullUrlBuffer[MAXBUF]; //Buffer for full url http://www.google.com:8080/index.html
	char parsedUrlBuffer[MAXBUF];	//Buffer to hold the parsed url www.google.com:8080/index.html
	char parsedPortNPath[MAXBUF];	//Buffer to hold the port and path :8080/index.html
	char port[MAXBUF];		//Buffer to hold the port number 8080
	char urlBuffer[MAXBUF];		//Buffer to hold the stripped url www.google.com
	char pathBuffer[MAXBUF];	//Buffer to hold the path and file name /index.html
	char clientRequest[MAXBUF];	//GET /index.html HTTP/1.1\r\nHost: wwww.rutgers.edu\r\n\r\n
	int n;	//arbitrary variable
	char GET[3];	//Holds GET
	char http[30];	//Holds HTTP/? information
	int portNumber = 80;	//Port number given by user; default set to 80
	char* token;
	int isPath;	//0 = path exists; 1 = path doesn't exist
